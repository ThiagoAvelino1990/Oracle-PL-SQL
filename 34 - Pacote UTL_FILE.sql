-------------------Pacote UTL_FILE-------------------
--O pacote UTL_FILE é um conjunto de componentes que tem como função permitir o acesso ou geração de arquivos externos ao banco de dados. 
...
--Realizar acesso ao SYS para dar GRANT ao pacote UTL_FILE para o seu usuário
...
BEGIN
 arquivo_saida := UTL_File.Fopen('DIRETORIO','Lista.xls','w'); --opção W para escrever no arquivo
...
DECLARE
 ...
 Linha Varchar2(100);
BEGIN
 arquivo := UTL_File.Fopen('DIRETORIO','Lista.txt', 'r'); --opção R para leitura do arquivo
 Loop
