...
--Para que o Oracle monte planos de execução otimizados, 
-- é necessário que as estatísticas dos objetos estejam sempre atualizadas. 
...
--As procedures desta package diferem do comando ANALYZE apenas pela possibilidade de permitir coletar estatísticas 
...
-------------------Package DBMS_STATS-------------------
--Permite coletar estatísticas exatas ou estimadas de objetos individualmente (tabelas, índices, cluster etc), 
...
--Gera historicos que são extremamente úteis para otimizar queries que efetuam pesquisas em colunas que possuem 
--  valores dispersos;
...
--A partir do Oracle Database 10G, as estatísticas são coletadas automaticamente pelo Oracle, 
-- diariamente de 2ª à 6ª, em um horário compreendido geralmente entre 22h e 2h, e aos sábados começa às 6h e termina somente no Domingo, às 2h.
--  É importante lembrar que ela só ocorre se o banco de dados estiver ocioso e somente nos objetos que tiveram mais que 10% de atualizações (inclui INSERT, UPDATE e DELETE). 
--  A partir do 11G, este valor de 10% é configurável.
--Pelo motivo dela ocorrer automaticamente, colete estatísticas somente quando você identificar alguma necessidade extra, 
--  como por exemplo, após uma carga de dados ou em banco de dados que trabalham 24X7 e que nunca ficam ociosos. 
--  Nestes casos, recomendo criar uma stored procedure contendo o código para coletar estatísticas de objetos do banco e criar em seguida um job para executar esta procedure periodicamente;
--Se o seu BD usa o CBO, evite coletar estatísticas através do comando ANALYZE TABLE e através da package DBMS_UTILITY.
--  Se você fizer isso, suas estatísticas serão menos precisas e você não terá historicos;
