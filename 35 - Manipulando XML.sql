...
--A XML (Extensible Markup Language) é uma linguagem de marcação de propósito geral. 
...
-- Usa um formato autodocumentado que contém a estrutura do documento, nomes de elemento e valores de elemento. Por causa dessas vantagens, a XML é muito usada para armazenamento e processamento de documentos, sendo aplicada por muitas organizações para o envio de dados entre seus sistemas de computador. 
...
--A função XMLELEMENT() é usada para gerar elementos XML a partir de dados relacionais. Você fornece um nome para o elemento e a coluna que deseja recuperar para a função XMLELEMENT() e ela retorna os elementos como objetos XMLType. 
--  XMLType é um tipo interno do banco de dados Oracle utilizado para representar dados XML. Por padrão, um objeto XMLType armazena os dados XML como texto em um CLOB (Character Large Object). O exemplo a seguir se conecta no SQL Developer com usuário de desenvolvimento e usa objetos XMLType. 
...
--OBS: Algumas quebras de linhas e espaços foram adicionados no código XML retornado por essa consulta para torná-lo mais fácil de ler. 
--  O mesmo foi feito em alguns dos outros exemplos deste capítulo. 

--  Você pode recuperar dados relacionais normais, assim como código XML, conforme mostrado no exemplo a seguir, que recupera a coluna cod_aluno como um resultado relacional normal e as colunas nome e cidade concatenadas como elementos XML:
...
--Você usa XMLFOREST() para gerar uma “floresta” de elementos XML. 
--  XMLFOREST() concatena elementos XML sem que você precise usar o operador de concatenação || com várias chamadas de XMLELEMENT(). 
...
--XMLELEMENT
--Com as barras || os dados aparecem na saída
...
------------------------
SELECT XMLPARSE(CONTENT'<TAluno><nome>Márcio Konrath</nome></TAluno>'WELLFORMED)||'' AS ALUNO
FROM dual;
...
--Gerando arquivo xml
DECLARE
  p_directory VARCHAR2(100) := 'DIRETORIO'; --Diretório do arquivo
  ...
  v_char_buffer VARCHAR2(32767); -- Tamanho máximo que pode usar
BEGIN
  ...
  UTL_FILE.PUT_LINE(v_file, '<?xml version="1.0"?>'); -- Gravar o cabeçalho do XML
