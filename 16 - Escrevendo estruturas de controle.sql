...
DECLARE
  vContador INTEGER;
BEGIN
  FOR vContador in 1..10 --Laço FOR de 1 até 10
  LOOP
    --vContador := vContador + 1;
...
BEGIN
  ...
  LOOP
    vContador := vContador + 1;
    IF (vContador Mod 2)=0 THEN   --Função MOD fará a divisão de vContador por 2, e mostrará o RESTO da divisão
      vTexto := 'Par';
    ELSE
      vTexto := 'Impar';
    END IF;
