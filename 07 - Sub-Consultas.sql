  ...
  -- Errado (Só pode retornar 1) Para o retorno de mais linhas, é necessário utilizar o operadores ("IN"; "NOT IN"; "EXISTS"; "NOT EXISTS")
  ...
  --estado do aluno 1
  --menos o aluno 1
  -- SUBCONSULTA, comparando com mais de uma coluna
  ...
  FROM TITEM
  WHERE cod_curso > 0
  GROUP BY COD_CURSO  --Agrupando colunas que não tem função de grupo
  HAVING Min(VALOR) >=
  ...
  --Operadores >; <; <=; <=; =; Trazem apenas um registro na SUBCONSULTA
  --Operadores IN; NOT IN; Trazem mais de um registro na SUBCONSULTA
  ...
  --- Este select também traz o mesmo resultado do SELECT acima, entretanto não é uma regra de negócio
  SELECT Cod_curso, nome, valor
  FROM Tcurso
  WHERE Cod_curso = 1
  OR Cod_curso = 2
...
-- Utilizando o operador IN para comparar DUAS colunas
