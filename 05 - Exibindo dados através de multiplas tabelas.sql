  ...
  SELECT TALUNO.COD_ALUNO, TALUNO.NOME, TCONTRATO.TOTAL
  FROM TALUNO, TCONTRATO
  WHERE TALUNO.COD_ALUNO = TCONTRATO.COD_ALUNO   --Critério de União. Join entre tabelas
  ...
  -- Errado - Coluna ambigua (COD_ALUNO) -> Esta coluna contém nas duas tabelas, e como não está defidino em qual tabela o SELECT deve pegar, é apresentado o erro de coluna ambígua
  ...
  --Uniao da tabela de aluno com contrato com apelido nas tabelas
  ...
  INSERT INTO TALUNO VALUES (10, 'PEDRO', 'NOVO HAMBURGO', NULL, NULL, NULL, SYSDATE)
...
----------------------------------------------------
  ...
  -------------
  SELECT	CON.COD_CONTRATO AS CONTRATO, CON.DESCONTO DESCONTO_CONTRATO,
...
COMMIT;
  ...
  SELECT Curso.Nome AS Curso, curso.PRE_REQ,
         Pre_Req.Nome AS Pre_Requisito, pre_req.cod_curso
